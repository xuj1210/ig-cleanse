...
function handleJSON(event) {
  ...
  if (MASTER_LISTS.relationships_followers && MASTER_LISTS.relationships_following) {
    ...
    for (const user of MASTER_LISTS.relationships_following) {
      if (!MASTER_LISTS.relationships_followers.has(user)) {
        listDisplay.append(usernameToLI(user));
      }
    }
  }
}
...
function cleanList(list) {
  return list.map(val => {
    const href = val.string_list_data[0].href.trim();
